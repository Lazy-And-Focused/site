  ...
  new Promise(async (res) => {
    for (const member of members) {
      try {
        ...
        if (!success || !Array.isArray(repos))
          throw new Error(JSON.stringify(repos, undefined, 4));
        ...
        for (const project of projects) {
          try {
            if (await Projects.findOne({ name: project.name }))
              throw new Error(`Проект ${project.name} уже сущесвует`);
