...
type IDocument<T> = Document<unknown, {}, T, {}> &
  T & { _id: ObjectId } & { __v: number };
...
class Parser<Model extends IModels> {
  ...
  public execute<T = IAllModels[Model]>(
    data: T | { [key: string]: unknown } | IDocument<T>,
  ): T {
    return Object.fromEntries(
      this.keys.map((k) => [k, (data as { [key: string]: unknown })[k]]),
    ) as T;
  }
}
