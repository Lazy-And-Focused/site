...
class Listener implements IListener<[IInteraction]> {
  ...
  public execute(data: Parameters<IListener<[IInteraction]>["execute"]>[0]) {
    return (interaction: IInteraction) => {
      if ("forward_origin" in interaction.update.message) {
        return forwardUtility.execute({
          type: "forwarded",
          message: interaction.update.message,
        });
      }
      ...
      const commandName =
        interaction.text.match(space) !== null
          ? interaction.text.slice(
              1,
              interaction.text.indexOf(
                (<RegExpMatchArray>interaction.text.match(space))[0],
              ),
            )
      ...
      if (!command)
        return interaction.reply(
          "ListenerError: Команда не была найдена.\nКоманда: " +
            commandName +
            "\nСписок команд:\n" +
