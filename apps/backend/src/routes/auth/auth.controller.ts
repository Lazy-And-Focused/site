...
export class AuthController {
  ...
  public async auth(
    ...
    if (method === "@me") {
      ...
      try {
        const data = await (await fetch(env.AUTH_SERVICE_URL + "/api/auth/" + method + "?code=" + req.query.code)).json();
    ...
    res.redirect(env.AUTH_SERVICE_URL + "/api/auth/" + method + "?callback=" + env.THIS_URL + "/api/auth/" + method + "/callback");
  }
