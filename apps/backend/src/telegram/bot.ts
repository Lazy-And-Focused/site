...
class Client {
  ...
  public launchListeners() {
    if (this.listenersInitialized)
      throw new Error("The listeners has initialized early.");
    ...
    Object.values(this.data.listeners).forEach((listener) => {
      ...
      this.bot.on(
        ...
        listener.execute({
          commands: new Map([
            ...(Object.keys(this.data.commands).map((c) => [
              c,
              this.data.commands[c],
            ]) as [string, ICommand][]),
          ]),
          help: new Map([
            ...(Object.keys(this.data.help).map((c) => [
              c,
              this.data.help[c],
            ]) as [string, any][]),
          ]),
  ...
  public callbackExecute<Data extends any[], Result>(
