...
class Utility<T extends { [key: string]: unknown }, K extends IModels> {
  public constructor(
  ...
  public async execute(interaction: IInteraction) {
    ...
    if (
      !env
        .get("TELEGRAM_BOT_ADMINS_ID")
        .split(",")
        .includes(`${interaction.from.id}`)
    ) {
      return interaction.reply(
        "Вас нет в списке администраторов LAF, вы не можете мной воспользоваться.",
      );
    }
    if (!interaction.text) {
    ...
    if (
      ...
      interaction.text.split(" ")[1] !== "delete"
    ) {
    ...
    try {
      if (action === "delete") {
        return interaction.reply(
          "Действие выполнено.\n" +
            JSON.stringify(
              await this.props.model.deleteOne({ name: name }),
              undefined,
              2,
            ),
        );
              ...
              JSON.parse(interaction.text.split("\n").splice(1).join("")).map(
                async (m: any) => {
                  ...
                  this.props.keys.forEach((k) => {
                    if (!(k in member))
                      throw new Error(`Ключ ${k} не найден в JSON`);
                  });
