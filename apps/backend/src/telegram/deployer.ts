...
const ignoreErrors = Boolean(
  env.get("IGNORE_TELEGRAM_DEPLOY_COMMANDS_ERRORS", true),
);
...
> {
  private readonly errorStack: Record<"paths" | "datas", Error[]> = {
    paths: [],
    datas: [],
  };
  ...
  private readonly ReadFolder = (
    path: string | string[] = this.datasPath,
    paths: string[] = this.datasPaths,
  ) => {
    ...
    for (const file of folder) {
      try {
        ...
        if (!existsSync(filePath)) {
          this.errorStack.paths.push(
            new Error("File at path " + filePath + " is not exists."),
          );
          continue;
        }
  ...
  private readonly Deploy = (paths: string[] = this.datasPaths) => {
    for (const path of paths) {
      const data = <T>(
        new (require(this.resolve(path)).default)(...this.constructorData)
      );
      const sendHelp = require(this.resolve(path))?.sendHelp;

      if (!("name" in data && "execute" in data)) {
        this.errorStack.datas.push(
          new Error("name or execute in file at " + path + " are not exists"),
        );
        continue;
      }
  ...
  public execute() {
    if (this.initialized)
      throw new Error("This deployer has initialized early.");
